...
deep_learning_models = ["fcn", "resnet", "inception", "mhattention", "attention", "fcnattention"]
tsc_models = ["rocket"]
...
def create_regressor(regressor_name, input_shape, output_directory, verbose=1, itr=1):
    ...
    if regressor_name == "mhattention":
        from models.deep_learning import mhattention
        return mhattention.MHAttentionRegressor(output_directory, input_shape, verbose)
    if regressor_name == "attention":
        from models.deep_learning import Attention
        return Attention.AttentionRegressor(output_directory, input_shape, verbose)
    if regressor_name == "fcnattention":
        from models.deep_learning import fcnattention
        return fcnattention.FCNAttentionRegressor(output_directory, input_shape, verbose)
