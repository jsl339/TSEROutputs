...
class MHAttentionRegressor(DLRegressor):
    """
    ...
    def build_model(self, input_shape):
        ...
        attention1 = self.transformer_encoder(conv1)

        conv2 = tf.keras.layers.Conv1D(filters=256,
                                       kernel_size=5,
                                       padding='same')(attention1)
        conv2 = tf.keras.layers.BatchNormalization()(conv2)
        conv2 = tf.keras.layers.Activation('relu')(conv2)

        attention2 = self.transformer_encoder(conv2)

        conv3 = tf.keras.layers.Conv1D(128,
                                       kernel_size=3,
                                       padding='same')(attention2)
        conv3 = tf.keras.layers.BatchNormalization()(conv3)
        conv3 = tf.keras.layers.Activation('relu')(conv3)

        dense = tf.keras.layers.Dense(10, activation="relu")(conv3)
